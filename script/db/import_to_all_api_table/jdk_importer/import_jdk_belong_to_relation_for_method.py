...
def is_imported(row_id):
    if KnowledgeTableColumnMapRecord.exist_import_record(session, jdk_method_knowledge_table, api_relation_table,
                                                         row_id,
                                                         "class_id"):
        return True
    else:
...
def import_all_jdk_method_belong_to_relation():
    # get all-version library
    cur.execute("select method_id,class_id from jdk_method")
...
def commit_to_server(result_tuples):
    ...
    for item in result_tuples:
        ...
        record = KnowledgeTableColumnMapRecord(jdk_method_knowledge_table, api_relation_table, old_id,
...
if __name__ == "__main__":
    import_all_jdk_method_belong_to_relation()
    cur.close()
