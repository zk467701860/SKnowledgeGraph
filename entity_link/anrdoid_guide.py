...
url_prefix = 'https://developer.android.com/guide/'
htmls = []
alias_url = []
alias_url_dict = {}
inner_alias_url = []
...
def get_url():
    ...
    res = requests.get(url_prefix, timeout=10)
    ...
    for a in soup.select('.devsite-section-nav a'):
        alias = a.string
        url = a['href']
    ...
    with open('raw_data_guide/urls.json', 'w') as f:
        json.dump(urls, f)
    return urls
...
def get_html(urls):
    ...
    for url in urls:
        ...
        html = str(soup.select('.devsite-article-body')[0])
        ...
        if counter % 100 == 0:
            ...
            with open("raw_data_guide/html.json", "w") as f:
                json.dump(htmls, f)
            with open("raw_data_guide/alias_link.json", "w") as f:
                json.dump(alias_url, f)
            with open("raw_data_guide/inner_alias_link.json", "w") as f:
                json.dump(inner_alias_url, f)
            with open("raw_data_guide/outer_alias_link.json", "w") as f:
                json.dump(outer_alias_url, f)
            with open("raw_data_guide/sucess_list.json", "w") as f:
                json.dump(sucess_list, f)  
            with open("raw_data_guide/failed_list.json", "w") as f:
...
if __name__ == "__main__":
    ...
    # # print(str(res.text))
    # soup = BeautifulSoup(res.text, 'lxml')
    # print(soup.select('.devsite-section-nav a'))
