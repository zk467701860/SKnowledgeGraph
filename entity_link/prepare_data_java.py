...
url_prefix = 'https://docs.oracle.com/javase/8/docs/api/'
htmls = []
alias_url = []
alias_url_dict = {}
inner_alias_url = []
...
def get_url():
    ...
    res = requests.get(url_prefix+'overview-frame.html', timeout=10)
    ...
    for a in soup.select("ul a"):
        alias = a.string
        url = url_prefix+a['href'].replace('package-frame', 'package-summary')
    ...
    # res = urllib2.urlopen(req)
    # data = res.read()
    res = requests.get(url_prefix+'allclasses-frame.html')
    soup = BeautifulSoup(res.text, 'lxml')
    # print(soup.find_all("a", target="packageFrame"))
    for a in soup.select("ul a"):
        alias = a.string
        ...
        urls.append(url)
    res.close()
    with open('raw_data_java/urls.json', 'w') as f:
        json.dump(urls, f)
    return urls
...
pattern3 = re.compile(r'(http://|https://)?docs\.oracle\.com.*')
...
def get_html(urls):
    ...
    for url in urls:
        ...
        contentContainer = str(soup.select('.contentContainer')[0]).replace(u'\u00a0', " ")
        html = header + contentContainer
        ...
        if counter % 100 == 0:
            ...
            with open("raw_data_java/html.json", "w") as f:
                json.dump(htmls, f)
            with open("raw_data_java/alias_link.json", "w") as f:
                json.dump(alias_url, f)
            with open("raw_data_java/inner_alias_link.json", "w") as f:
                json.dump(inner_alias_url, f)
            with open("raw_data_java/outer_alias_link.json", "w") as f:
                json.dump(outer_alias_url, f)
            with open("raw_data_java/sucess_list.json", "w") as f:
                json.dump(sucess_list, f)  
            with open("raw_data_java/failed_list.json", "w") as f:
